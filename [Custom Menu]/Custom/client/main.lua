...
local shopProfit = Config.shopProfit['mechanic']
...
function resetMenu()
	if not mainMenu and PlayerData.job then
		...
		if PlayerData.job.name == 'mechanic' then
			mainMenu = RageUI.CreateMenu("Benny's Motor Works", "~b~Action")
...
function CreateLSMenu()
	if not bodyMenu then
		bodyMenu = RageUI.CreateSubMenu(mainMenu, "Carosserie", "Gestion de la carosserie")
		bodyMenu.EnableMouse = false

	end
	if not recapmenu then
		recapmenu = RageUI.CreateSubMenu(mainMenu, "Récapitulatif", "Action")
		recapmenu.EnableMouse = false
	end
	if not extrasMenu then
		extrasMenu = RageUI.CreateSubMenu(mainMenu, "Extras", "Gestion des extras")
	...
	if not colorMenu then
		colorMenu = RageUI.CreateSubMenu(mainMenu, "Couleurs", "Gestion des couleurs")
		colorMenu.EnableMouse = false
	end
	if not neonMenu then
		neonMenu = RageUI.CreateSubMenu(mainMenu, "Neon", "Gestion des neons")
		neonMenu.EnableMouse = false
	end
	if not upgradeMenu then
		upgradeMenu = RageUI.CreateSubMenu(mainMenu, "Amélioration", "Gestion des ameliorations")
		upgradeMenu.EnableMouse = false
	end
	if not cartMenu then
		cartMenu = RageUI.CreateSubMenu(mainMenu, "Panier", "Liste des achats")
	...
	while true do
		...
		if IsPedInAnyVehicle(playerPed, false) then
			...
			if IsControlJustReleased(0, 38) and not lsMenuIsShowed and isInLSMarker then
				if (PlayerData.job and (PlayerData.job.name == 'mechanic')) or not Config.IsMecanoJobOnly then
					...
					if ((vehicleClass ~= 8 or not Config.IsMotorCycleBikerOnly) and PlayerData.job.name == 'mechanic') or 
						((vehicleClass ~= 8 or not Config.IsMotorCycleBikerOnly) and PlayerData.job.name == 'mechanic') or
			...
			RageUI.IsVisible(extrasMenu, function()
					...
					RageUI.Slider("Couleur plaque", newPlateIndex, 6, nil, false, {}, true, 
						{
			...
			RageUI.IsVisible(recapmenu, function()
				local menuItemCount = 0
				for k, v in pairs(shopCart) do 
			...
			RageUI.IsVisible(cartMenu, function()
				...
				if menuItemCount == 0 then
					...
					RageUI.Button("Récapitulatif de commande", nil, {RightLabel = "→→"}, true, {
						onSelected = function()
						end
					   }, recapmenu)
					RageUI.line(255,255,255,255)
